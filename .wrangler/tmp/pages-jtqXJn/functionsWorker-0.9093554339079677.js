...
async function handleChat(messages, apiKey, model = "gpt-4o-mini", apiUrl) {
...
__name(pickNonEmpty, "pickNonEmpty");
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  try {
    ...
    const model = openRouterKey ? pickNonEmpty(
      env.OPENROUTER_MODEL,
      env.openrouter_model,
      env.OPENAI_MODEL,
      env.openai_model
    )?.trim() : pickNonEmpty(env.OPENAI_MODEL, env.openai_model, env.OPENROUTER_MODEL, env.openrouter_model)?.trim();
    const apiUrl = openRouterKey ? "https://api.openrouter.ai/v1/chat/completions" : void 0;
    const reply = await handleChat(messages, apiKey, model, apiUrl);
    return Response.json({ reply }, { status: 200, headers: cors2 });
  } catch (e) {
...
// ../.wrangler/tmp/pages-jtqXJn/functionsRoutes-0.06925919010140036.mjs
var routes = [
  {
    routePath: "/api/chat",
    mountPath: "/api",
